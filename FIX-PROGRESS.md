...
- [~] **Phase 3 — de-dup writers + locking**
  - [x] **K08** real `flock`: `make apply` acquires exclusive `.publish.lock` then re-execs `apply-locked`; `_rollback.py` force-push takes the same lock; `sync-catalog --deploy` inherits it via `make apply`.
...
- [~] **Phase 5 — regenerate, verify green, deploy**
  ...
  - [x] **DEPLOYED 2026-06-12** (owner-authorized): 20 app-repo re-signs pushed to origin/publish; branch merged → main (origin/main `2a07f6e9`); deployer manifest reseated (15 hashes + DueProcess v1 deferred, `pending_reseat=true` flags the owed on-chain GlobalAppApproval reseat); all 6 preflight gates passed; `make apply` force-pushed publish→gh-pages (`8ef28923`). **Live CDN verified: 43 apps, 0 stubs.** Rollback armed: `git push -f origin publish-prev:publish` (publish-prev=`cd72dac4`).
  - **Owed follow-up (deployer authz):** on-chain GlobalAppApproval reseat for the `pending_reseat` apps — until then the runtime deployer may reject installs for re-hashed apps (catalog + on-chain ReleaseEntry are correct; this is the separate install-authz seat).
