...
>
> **Non-goals:** preserving any existing pearl state, migrating users across schema versions, keeping deprecated APIs alive. Every pearl boots empty.
...
3. **`pearl-crypto-journal`** is already under the correct namespace (`github.com/hrbrlife/grain-crypto-journal`). Canonical working copy: `/home/user/Desktop/Melusina/shared/grain-crypto-journal/`. Vendored into 27+ apps via submodule + in-tree copies that must be deleted.
4. **`pearl-e2e-binding`** is at `/home/user/Desktop/Melusina/shared/grain-e2e-binding/` as `github.com/melusina-os/grain-e2e-binding` — a **different wrong namespace** from grainrestore's `melusina/`. Both must move to `hrbrlife/`.
...
8. **Sandstorm restore mechanics:** backup is an unencrypted ZIP of `/var` + `GrainInfo`; restore mints a new grainId; no restore hook. GrainId is read from `/var/grainid` (and `SANDSTORM_GRAIN_ID` env fallback). `X-Sandstorm-Pearl-Id` HTTP header does **not** exist.

---

## 1. Target component roster (final state)

**Eleven repos** under `github.com/hrbrlife/`. Every pearl-packaged app consumes `melusina-spkmodule-component` as a submodule; implicit from here on.
...
| 2 | `melusina-capnp` | `github.com/hrbrlife/melusina-capnp` | Single source of truth for every `.capnp` schema: `pearl-authz`, `botmother`, `agentchat`, `payment`, `wallet`, `audit`, `ai`, `kyc`, `notification`, `storage`, `identity`, `station`, `supervisor`, `telegram`, `static-publish`, `document`, `workflow`, `instance`, `testcap`, `gohtmx`, `casemeta` (21 schemas). Auto-generated Go/TS/Py bindings via CI. | — |
| 3 | `melusina-pearl-crypto-journal-component` | `github.com/hrbrlife/grain-crypto-journal` | `keybox.Manager` (Rewrap/ClearAuthorizedKeys/ResetBinding/UnsealPathB), `sqlitestore.Store` (AES-256-GCM + hash-chained `Audit`), binding state machine, JS twin. | — |
...
| 6 | `melusina-pearl-auth-component` | `github.com/hrbrlife/melusina-pearl-auth/go` + npm `@melusina/grain-auth` + PyPI `melusina-pearl-auth` | Three-language Cap'n Proto admin authz sidecar client. Fail-closed middleware. ≤500 ms UI perms TTL. Ed25519 verification. Schema-coherence CI. | #2, #3, #4, #5 |
| 7 | `melusina-http-component` | `github.com/hrbrlife/melusina-http-component/go` | Canonical Sandstorm HTTP-out + Grapple. `BuildHTTPOutDescriptor`, 4-variant sidecar selector, `PowerboxManager` with mandatory `SandstormApi.drop(token)`. | #3 |
| 8 | `melusina-pearl-e2e-binding-component` | `github.com/hrbrlife/grain-e2e-binding` | E2E wallet-binding protocol: keybox ↔ wallet for Path-B recovery, binding/migration state, authorized-key sync. | #3, #4 |
| 9 | `melusina-pearl-restore-component` | `github.com/hrbrlife/melusina-pearl-restore` | **v0.1.0 scope: same-machine restore only** (persona remap). `Open/Register*/Mount/CaptureOnBackup/RewriteOnRestore`, dispatch matrix, manifest. **Both current bugs fixed** (Sec. 4). Cross-machine migration deferred to v1.0.0 per Sec. 4.6. | #3, #8, #6 |
| 10 | `melusina-e2e-test-component` | PyPI `melusina-e2e` | Browser-driven E2E harness, screenshot store, Sandstorm shell helpers. Orchestrates 23-scenario constellation tests including TeleScreen C4. | `melusina-pearl-auth-py` |
...
- `melusina-static-publish`: **Service-tier consumer** — `melusina_botmother` (full `service.Service` delegation via journal adapter). **Primitives-only consumers** — `cyberteller` (uses HMAC + path helpers, does its own write via `FilePublisher`). Deferred: pearl-instance template.
...
|---|---|---|---|---|
| pearl-crypto-journal | `Melusina/shared/grain-crypto-journal/` | `github.com/hrbrlife/grain-crypto-journal` | ✓ | **SHIPPED** — v0.1.0 + v0.1.1 tagged and pushed. 15+ consumers still use `v0.0.0 + local replace`; bumping each is consumer-by-consumer port work (see instaco.app / vintage-test-dec below), not a mass task. |
| grainrestore | `/grainrestore/` → now `hrbrlife/melusina-pearl-restore` | `github.com/hrbrlife/melusina-pearl-restore` | ✓ | **SHIPPED** — v0.1.0 + v0.1.1 tagged. Consumers: ccash + 2 NamedCoin apps. |
| pearl-e2e-binding | `Melusina/shared/grain-e2e-binding/` (monorepo copy) + standalone at `hrbrlife/grain-e2e-binding` | `github.com/hrbrlife/grain-e2e-binding` (standalone) | ✓ | **SHIPPED** — standalone v0.1.0 live. Monorepo copy still declares `melusina-os/` but is effectively legacy; `hrbrlife/` is canonical. 3 consumers (instaco.app, vintage-test-dec, store-rebuild duplicate) still pin `melusina-os/ v0.0.0 + replace` — consumer port jobs, not a tag op (API diverged: `UserContext.WalletChain`, `HasLegacyPIN`, `WalletPubkey`, `keybox.GetUserKey`, `RegisterWalletUser` all removed before v0.1.0). |
...
| melusina-pearl-auth | `/melusina-pearl-auth/` | `github.com/hrbrlife/melusina-pearl-auth/go` + `@hrbrlife/grain-auth` (npm, package.json 0.1.1) + `melusina-pearl-auth` (PyPI, pyproject 0.1.1) | ✓ | **SHIPPED (Go)** — `go/v0.1.0` + `go/v0.1.1` Git tags live; Go proxy resolves. Consumers: cyberteller + ai-lagoon pinned at v0.1.1. **npm publish pending** user auth (dry-run `npm pack` green — 16 files, 7.7 kB). **PyPI publish pending** user auth (wheel+sdist built at `py/dist/`). |
| melusina-http-component | `/melusina-http-component/` | `github.com/hrbrlife/melusina-http-component/go` | ✓ | **SHIPPED** — v0.1.0 tagged (`v0.1.0` + `go/v0.1.0`); Go proxy resolves; `pearl-crypto-journal` pinned at v0.1.0 (no sibling replace). Consumers: cyberteller + ai-lagoon pinned at v0.1.0. |
...
| App path | Git remote | Branch | grainrestore | pearl-crypto-journal | Other shared | Status |
|---|---|---|---|---|---|---|
...
| `/Desktop/cyberteller` | hrbrlife/cyberteller | feat/admin-auth-harmonize | ✗ | ✓ (indirect via http-component) | melusina-pearl-auth, melusina-http-component | **PARTIAL** |
| `/Desktop/ai-lagoon` | hrbrlife/ai-lagoon | feat/admin-auth-harmonize | ✗ | ✓ (indirect via http-component) | melusina-pearl-auth, melusina-http-component | **PARTIAL** |
...
**ccash_go_htmx**
- `pearl-crypto-journal/keybox/fields.go` lines 13, 57–60: "ENC1:" plaintext-fallback prefix + `DecryptEventData` compat branch. **Strip** — greenfield keybox is always ciphertext.
- `pearl-crypto-journal/keybox/binding_test.go`: `MigrationState`/`MigrationComplete` assertions. **Strip** — no migration state in greenfield.
...
- Committed `backup.zip` / `sample-pearl.zip` fixtures in prior format
...
**Scope locked:** v0.1.0 handles **same-machine restore only** (persona remap after the user remounts the pearl on the same Melusina host). Cross-machine pearl migration — the DNS-TXT-pinning / verifyDestination / pearl-migration-receive protocol referenced in prior stubs — is **deferred to v1.0.0** per the design foreseen in `/Desktop/Melusina/sandstorm/`. Those stubs do not actually exist in the current Sandstorm source; the "transfer" codepath in `transfers-server.js` reuses the same backup-restore flow (new grainId + persona remap) as local restore.

Both bugs below must be fixed **before** the repo is renamed to `hrbrlife/melusina-pearl-restore-component` and tagged v0.1.0. Fanning out a broken restore component to more apps multiplies the bug.
...
- `NamedCoin-admin-app` — no resign hook needed (admin app doesn't hold pearl keys).
...
Not v0.1.0. When implemented (v1.0.0), the protocol lives in the Sandstorm shell (`/Desktop/Melusina/sandstorm/src/sandstorm/`) not in the pearl component. The pearl-side contract is: accept a migration-bundle envelope at `/grainrestore/migrate-receive`, verify signed receipt + destination attestation, then run the same ceremony as local restore. Kill list is explicit this is future work; the library v0.1.0 does NOT expose a migrate endpoint.
...
| 0.2 | Build `tools/schema-coherence.sh` for Go↔TS↔Py↔capnp parser comparison | Script lives in `melusina-pearl-auth-component`; green on existing repo state | 0.5 |
...
| 1.1b | Create `github.com/hrbrlife/melusina-pearl-restore-component` repo; `git mv` source; `gofmt -r 'github.com/melusina/grainrestore -> github.com/hrbrlife/melusina-pearl-restore' -w ./...`; `go mod tidy`; update 3 consumer `replace` directives | 0.5 |
| 1.1c | Tag v0.1.0 (see Sec. 6 "first-tag workflow") | — |
| 1.2 | Promote `pearl-crypto-journal` out of `Melusina/shared/` → `hrbrlife/melusina-pearl-crypto-journal-component`; preserve import path `github.com/hrbrlife/grain-crypto-journal` (rename the repo, not the module); tag v0.1.0 | 0.5 |
...
| 1.7 | **NEW — Create `hrbrlife/melusina-capnp` as Component #2.** Seed from `melusina_botmother/capnp/` (20 schemas) + `melusina-pearl-auth/capnp/grain-authz.capnp` = 21 canonical schemas under `interfaces/`. Add Go/TS/Py binding generators (`capnp compile`, `capnpc-ts`, `pycapnp`) wired into CI. Update `melusina_botmother`, `melusina_teleport2`, and `melusina-pearl-auth` consumers to import from the canonical repo; delete their in-tree capnp copies. Tag v0.1.0 | 1.5 |
...
| # | Task | Blocker | Days |
|---|---|---|---|
| 2.1 | Rename `pearl-e2e-binding` → `hrbrlife/melusina-pearl-e2e-binding-component`. `gofmt -r 'github.com/melusina-os/grain-e2e-binding -> github.com/hrbrlife/grain-e2e-binding' -w ./...` across consumers; drop sibling `replace`s; tag v0.1.0 | 1.2, 1.3 | 1.0 |
| 2.2 | Finalize `melusina-http-component` as standalone: drop sandstorm + pearl-crypto-journal sibling `replace`s; pin tagged pearl-crypto-journal; tag v0.1.0 | 1.2 | 0.75 |
| 2.3 | `melusina-pearl-auth-component` v0.1.0 publish — simultaneous **Go tag → npm → PyPI** (in that order; only npm/PyPI are irreversible; dry-run against verdaccio + TestPyPI first) | 1.5, 2.1, Sec. 8 #4 | 1.5 |
...
|---|---|---|---|
| 3.A | ccash_go_htmx | **Adopt `melusina-http-component` #7 — no exemption.** Delete `internal/httpout/` if present; migrate Grapple + HTTP-out to the shared component. Consumer side of Bug A/B fixes from Sec. 4.4. Delete `Fineract-sidecar/upstream/mifos-web-app/`. | 1.5 |
...
| 3.E | sailsto_system + AITX Procedures (+ clientspace, telescreen-pearl) | sailsto: reconcile 3 dirty + 1 unpushed first. AITX: telescreen-pearl is Python → adopts `melusina-pearl-auth-py` + `melusina-identity-gate-py` | 2.0 |
| 3.F | openclaw-main + client_collection + MiniGit | MiniGit: reconcile dual-branch first (Sec. 3.2). openclaw: Go bridge adopts pearl-crypto-journal + http-component + pearl-auth; JS side adopts `@melusina/grain-auth` + `@hrbrlife/bureau-shell` if waikiki-template editing overlaps | 2.25 |
| 3.G | 4 bureau apps + melusina-mermail-station-app | **Bureau apps adopt `@hrbrlife/bureau-shell` npm + `pearl-crypto-journal` + `http-component`.** Each app ships its `DocumentAdapter<S>` module per Sec. 11.4. mermail-station-app: adopt pearl-crypto-journal + pearl-auth + identity-gate; migrate novel content from INSTASYS_MAIL before Bucket 4 archives it | 2.5 |
...
- `GRAIN_CRYPTO=stateless` → artifact does NOT import pearl-crypto-journal AND does NOT reference `MELUSINA_VAULT_SECRET` AND writes only static files under `/var`. Self-certification without static confirmation is rejected.
...
- Re-implementing keybox, sqlitestore, Grapple persistence, authz client, envelope verification, restore driver, HTTP-out in-app.
...
- **Schema coherence** (pearl-auth, identity-gate): `tools/schema-coherence.sh` — Go/TS/Py/capnp must match.
...
- Injected by Sandstorm per-pearl env at install.
...
Every pearl boots empty — no prior state to recover. Per-pearl first-deploy:
1. User installs pearl from bazaar.
2. Pearl init detects missing `/var/keybox.json` → enters enrollment UI.
...
If a pearl is *restored* (new grainId, no Path-B wallet): restore ceremony in #8 library detects the condition; surfaces `restored-pending` state; app renders a re-enrollment UI rather than booting normally.
...
| 11 | Canonical `.capnp` schema root | **CREATE `hrbrlife/melusina-capnp` as new Component #2.** Migrate 21 schemas from `melusina_botmother/capnp/` (20) + `melusina-pearl-auth/capnp/` (1). Delete duplicate in `melusina_teleport2/capnp/`. Task 1.7. |
...
Day 3:    Task 1.2 pearl-crypto-journal promote         [0.5d]
Day 3.5:  Task 1.7 create melusina-capnp (NEW)          [1.5d]
Day 5:    Task 2.3 pearl-auth (Go+npm+PyPI)             [1.5d]
...
- **Lane P2:** Task 2.1 (pearl-e2e-binding rename), 2.2 (http-component finalize), 2.4 (bureau-shell extraction), 2.5 (e2e-test tag) — 3.5d, starts after 1.2+1.3 land.
...
Affected apps: `openclaw-main` (JS + Go bridge), `melusina-bureau-*-app` (TS/React), `INSTASYS_CHAT` (deferred), and the browser twin of pearl-crypto-journal.
...
Discipline:
- PyPI packages published under their own names (`melusina-pearl-auth`, `melusina-identity-gate`, `melusina-e2e`).
- `setup.py` / `pyproject.toml` pins via `melusina-pearl-auth==0.1.0` — no `>=`.
...
Snapshots table lives in the pearl's `sqlitestore.Store` (Component #3), not a standalone DB — keeps hash-chained audit integration free.
...
CREATE TABLE snapshots (
  ...
  created_by        TEXT NOT NULL,           -- pearl user id
...
- Cross-pearl snapshot transfer
- Encryption at rest for blobs (hash is integrity, not confidentiality — pearl-crypto-journal handles confidentiality layer-above)
...
- Lifecycle: pearl launches sidecar on boot (`exec` or goroutine); sidecar terminates on pearl SIGTERM
- Restart: sidecar crash does NOT restart pearl; pearl auth client retries with exponential backoff
- Capability forwarding: Sandstorm caps (HTTP-out, Grapple tokens) pass via capnp to sidecar; sidecar never calls Sandstorm APIs directly
...
| `telescreen-companion-app` | `/Desktop/Melusina/sidecar/telescreen-companion-app/` | **ADJACENT** | Companion setup pearl for TeleScreen config distribution. No adoption work — already aligned. |
...
Days 1.5–7 — Bucket 1 (foundations, 11-component roster)
  ...
  1.1b rename → hrbrlife/melusina-pearl-restore    [0.5d]
  1.1c tag v0.1.0
  1.2  pearl-crypto-journal promote (#3)            [0.5d]
...
Days 7–12 — Bucket 2 (wrappers)
  2.1 pearl-e2e-binding rename (#8)                 [1.0d]
  2.2 http-component finalize (#7)                  [0.75d, parallel]
  2.3 pearl-auth Go+npm+PyPI (#6)                   [1.5d]
...
Scope locked:
  - pearl-restore v0.1.0 = same-machine restore only.
