...
| `MELUSINA_BUNDLE_TARBALL`, `MELUSINA_BUNDLE_UPDATE_TOOL`, `MELUSINA_BUNDLE_UPDATE_KEYRING`, `MELUSINA_BUNDLE_INSTALL_SH` (optional) | unset | The Sandstorm bundle update is built only when `MELUSINA_BUNDLE_TARBALL` names a `sandstorm-<build>.tar.xz`. Each named input needs its `<NAME>_SHA256` pin (`scripts/release-inputs.py digest <path>`); a missing or mismatched one is refused by name before any work. Unset, no bundle is built and the live `update/` tree is carried across. `SANDSTORM_SRC` (a scanned sibling checkout) and `DEPLOY_UI_SRC` are retired and refused by name. |
| `MELUSINA_SKIP_BUNDLE_UPDATE` | unset | Set to `1` to skip the Sandstorm bundle-update block even when a bundle is named, and refresh the live `update/` payload instead (catalog ships, no new `update/sandstorm-N.tar.xz.update-sig`). |
...
| `MELUSINA_DEPLOYER_MANIFEST` | unset | Path to the deployer's approval manifest, pinned by `MELUSINA_DEPLOYER_MANIFEST_SHA256`. Read by `scripts/preflight.sh` Gate 2 (skipped when unset) and `scripts/doctor.sh`, edited by `scripts/manifest-merge.sh` (which requires it and prints the new digest). |
