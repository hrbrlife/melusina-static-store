...
> §4.4 (admin grain state) and §5.A (admin task list A1–A7).
...
1. **A1 done** — `melusina_ccashconfig_app/main.go` is no longer the 55-LOC welcome-page stub. A real Cap'n Proto `AdminGate_Server` is wired in front of `pkg/registry`, `pkg/router`, `pkg/broker`, `pkg/manifest`. Source: kill-list §5.A row A1. *(Status note 2026-04-25: `main.go` has grown to ~207 LOC with manifest/registry/router/broker bootstrap and `/admin/cases` + `/admin/spawn` http-bridge endpoints; the Cap'n Proto AdminGate offerable surface is the remaining piece. Re-verify against the kill-list §4.4 audit and §5.A A1 row before declaring this precondition green — the audit snapshot ages quickly while A1 lands.)*
2. **A2 done** — `sandstorm-pkgdef.capnp` exports the `AdminGate` offerable. Without this, ccash Powerbox cannot claim it. Source: kill-list §5.A row A2.
3. **A3–A5 done** — manifest allowlist wired (§1.5 invariant), `getFlowTemplate` reads YAML from `ccash_domain_template/domains/popaye/stations/*.yaml`. *(A4 amendment 2026-04-25: the original `DEV_MODE=1` env-var-gated re-read was superseded by mtime-aware cache invalidation in `melusina_ccashconfig_app/pkg/server/template_provider.go` — commit `a0ec71c`, accepted by Riker chat idx 102. Operator UX is unchanged: edit a YAML, restart the ccash grain, the new content is served on next handshake. The catalog metadata.json contract is unaffected; this footnote exists so a future reader doesn't chase a `DEV_MODE` flag that doesn't exist.)*
...
git commit -m "catalog: add cca.sh Config v0.1.0 (admin-grain landing, kill-list M1)"
