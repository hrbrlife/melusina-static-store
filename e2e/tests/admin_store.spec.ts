...
test.describe('Admin app market', () => {
  ...
  test('Admin endpoint serves at least one of our installable packages', async ({ request, page }) => {
    ...
    const cat = await (await ctx.get('https://hrbrlife.github.io/melusina-static-store/apps/index.json')).json();
    const app = cat.apps[0];
    const installUrl = new URL(`/install/${app.packageId}`, ADMIN);
    installUrl.searchParams.set('url', `https://hrbrlife.github.io/melusina-static-store/packages/${app.packageId}`);
