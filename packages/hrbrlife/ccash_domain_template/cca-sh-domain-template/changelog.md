...
- Grapple picker copy rewritten from pearl/capnp developer language
...
- Grapple picker now renders the bound version + first-8 of the
...
- README documents that DueProcess / popaye / cyberteller / Welcome
  pearl require a launched Template Authority pearl in the Sandstorm
  instance before their bind flows can succeed (no DTG ⇒ Grapple
  returns "No token received from Grapple" and the user is forced
  into a blank-builder fallback).

## 0.1.0 — 2026-04-23

Initial release. Ported as a standalone package from the AITX Procedures
monorepo, where it lived as the `template` pearl type of `bloom-process`.
