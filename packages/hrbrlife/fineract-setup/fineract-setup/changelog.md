...
- **Multi-sidecar endpoint selector on Step 0 (connectivity check).**
  ...
  which Fineract-sidecar variant this pearl should reach: `host`,
...
- **Pre-save reachability probe.** After the Grapple grant lands
  ...
  transport is swapped, the pearl issues a probe GET through the
...
- Wired the `Fineract-setup` companion pearl into a Sandstorm package
- Added `launcher.sh` + `sandstorm-pkgdef.capnp` using the ccash pearl pattern
