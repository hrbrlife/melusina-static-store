# melusina-app-creeper — OSINT Consumer Pearl

Identity: **melusina-app-creeper** (Sandstorm consumer pearl SPK).
AppId: `jkt51nyrcp2wk6n4atseexddjv9wnyrzu3ypmdp2qzk6d9n2q4eh`
Version: v1 (`0.1.0`, initial split from telescreen).
Stack: Go pearl (Cap'n Proto UiView under `-tags=melusina`, FD3), no
bundled sidecar.

## PSP Role

This pearl is the **consumer-pearl front for OSINT** in the Melusina
PSP stack. It exposes a `CreeperService` Grapple capability that
...
- Do NOT bypass the authz path. Every verb is fail-closed: if the
  Grapple cap is not held, return `Status_failed` (CLAUDE.md
