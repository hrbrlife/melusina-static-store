...
- HiDPI grain + appGrid icons. Switched pkgdef icons block from
...
- **Imperative #22 — TeleScreen Hub**: greenfield reset to a new app
  with new appId minted via `spk keygen`. The two-grain TeleScreen
...
- Native-capnp Go core grain landed (not yet the production
  wire — pkgdef `coreCommand` still execs Python). Binary is
  staged alongside at `/opt/telescreen/grain` for pre-flip
  smoke. Full architecture: `grain/README.md`. Flip commit
