...
# --runtime-contract is REQUIRED by the submit client: RELEASE.json binds the
# contract's sha256 and the contract binds sha256(SPK), so the store's
# check=runtime_contract gate can only pass when the exact release-bound
# artifact travels with the publish. A slot without one fails closed here
# rather than being silently published as uncertified.
submit_common=(
