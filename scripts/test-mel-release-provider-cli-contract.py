...
def test_release_status_uses_live_release_v2_layout():
    version = b"2.0.26"
    raw = bytearray(383)
    raw[:8] = provider.RELEASE_ENTRY_DISCRIMINATOR
    raw[40:72] = bytes.fromhex("ab" * 32)
    raw[136:140] = len(version).to_bytes(4, "little")
    raw[140:140 + len(version)] = version
    status_offset = 140 + len(version) + 32 + 32 + 64 + 32 + 32 + 8
    value = {
        "owner": "program",
        "data": [provider.base64.b64encode(raw).decode(), "base64"],
    }
    for status, label in enumerate(("Active", "Revoked", "Superseded")):
        raw[status_offset] = status
        value["data"][0] = provider.base64.b64encode(raw).decode()
        assert provider.decode_release_status("release-pda", value, "program") == {
            "pda": "release-pda",
            "appHash": "ab" * 32,
            "version": "2.0.26",
            "status": label,
    ...
    wrong_owner = dict(value, owner="other-program")
    try:
        provider.decode_release_status("release-pda", wrong_owner, "program")
    except provider.ProviderError as exc:
        assert "owner" in str(exc), exc
    else:
        raise AssertionError("foreign-owner ReleaseEntry was accepted")

    bad_discriminator = bytearray(raw)
    bad_discriminator[:8] = b"notentry"
    wrong_kind = dict(value, data=[provider.base64.b64encode(bad_discriminator).decode(), "base64"])
    try:
        provider.decode_release_status("release-pda", wrong_kind, "program")
    except provider.ProviderError as exc:
        assert "discriminator" in str(exc), exc
    else:
        raise AssertionError("wrong-account discriminator was accepted")
...
if __name__ == "__main__":
    ...
    test_release_status_uses_live_release_v2_layout()
