...
func TestAppCatalogRecoveryColdRebuildRestoresMissingV2RuntimeContract(t *testing.T) {
	root := t.TempDir()
	registerRecoveryRootCleanup(t, root)
	pub, priv, _ := ed25519.GenerateKey(rand.Reader)
	id := appCatalogGenerationPrefix + strings.Repeat("9", 32)
	writeRecoveryGenerationV2(t, root, id, []string{"app-one"}, "1.0.0", priv)
	if err := os.Symlink(id, filepath.Join(root, appCatalogCurrentLink)); err != nil {
		t.Fatal(err)
	}
	contractPath := filepath.Join(root, id, "attest", "app-one", "RUNTIME-CONTRACT.json")
	if err := os.Chmod(filepath.Dir(contractPath), 0o755); err != nil {
		t.Fatal(err)
	}
	...
	if got.ID == id {
		t.Fatal("cold recovery selected the incomplete generation without rebuilding it")
	}
	_, _, _, want, _ := recoveryReleaseBytesV2(t, "app-one", "1.0.0")
	served, err := readSnapshotFileExact(got, "attest/app-one/RUNTIME-CONTRACT.json", int64(len(want)))
	if err != nil {
	...
	if !bytes.Equal(served, want) {
		t.Fatal("cold-rebuilt generation did not restore the exact staged runtime contract")
	}
}
