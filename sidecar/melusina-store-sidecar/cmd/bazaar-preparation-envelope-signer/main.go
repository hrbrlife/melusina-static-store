// Command bazaar-preparation-envelope-signer owns the preparer-side publisher
...
func main() {
	if err := run(os.Args[1:]); err != nil {
		fmt.Fprintln(os.Stderr, "bazaar-preparation-envelope-signer:", err)
		os.Exit(1)
	}
}

func run(args []string) error {
	fs := flag.NewFlagSet("bazaar-preparation-envelope-signer", flag.ContinueOnError)
	...
	uidText := fs.String("preparer-uid", "", "numeric Unix uid allowed to request envelopes (required)")
	if err := fs.Parse(args); err != nil {
		return err
	}
	if *uidText == "" {
		return fmt.Errorf("-preparer-uid is required; do not default a custody peer")
	}
	uid, err := strconv.ParseUint(*uidText, 10, 32)
	if err != nil {
		return fmt.Errorf("-preparer-uid: %w", err)
	}
	service, err := preparationenvelope.Load(preparationenvelope.Config{
	...
	return preparationenvelope.Serve(context.Background(), *socket, service)
}
