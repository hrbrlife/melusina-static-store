...
func TestLoadFamilyRealManifest(t *testing.T) {
	...
	wants := map[string]want{
		...
		"ccashconfig":        {"money-path", "6gdgveudrer5a61hp8qkmxcn89wyce5uq1mg92ud40ugr2uj7mz0", "ccashconfig", "fe3f4f9aada9bab9ea4ff5d5d5e5393b509e9e84", "hrbrlife", "melusina_ccashconfig_app", "cca-sh-config", ""},
		...
		"creeper":            {"platform-tools", "pm1afskzvf2vfasvxhwktk0u0sq7um0942psrdzdhf7w463n92eh", "creeper", "3127c8fe2f092c26a299f400acb38f336c646686", "hrbrlife", "melusina-app-creeper", "creeper", ""},
		...
		"bureau-calendar":    {"bureau-rich-office", "p0wjp099ry06x0shap6ts270x55tn24pa5pt5029qdyhpqkaztv0", "bureau-cal", "dbd4590c599b2708f7f4c8b5786f951a520a9c99", "hrbrlife", "melusina-bureau-cal-app", "bureau-cal", ""},
		"bureau-contacts":    {"bureau-rich-office", "trymnqgywrmc3pskv6160e7h2gjscm9kentjkeah6pnvyeqeq0kh", "bureau-contacts", "fc847592758a5772179d31c0015031a2ca2673ef", "hrbrlife", "melusina-bureau-contacts-app", "bureau-contacts", ""},
	...
	for _, app := range fam.Apps {
		...
		seen[app.Name] = true
		if app.SourcePath == "" || filepath.IsAbs(app.SourcePath) || filepath.Clean(app.SourcePath) != app.SourcePath || strings.Contains(app.SourcePath, "\\") {
			t.Fatalf("production manifest entry %q has unsafe source_path %q", app.Name, app.SourcePath)
		}
		for _, segment := range strings.Split(app.SourcePath, "/") {
			if segment == "" || segment == "." || segment == ".." {
				t.Fatalf("production manifest entry %q has unsafe source_path %q", app.Name, app.SourcePath)
			}
		}
		if app.CatalogDeveloper == "" || app.CatalogRepo == "" || app.CatalogSlug == "" {
			t.Fatalf("production manifest entry %q has incomplete catalog coordinates", app.Name)
		}
