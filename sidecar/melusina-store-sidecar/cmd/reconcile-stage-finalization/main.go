...
func validateStageRecord(dir string, record stageRecord, staged release, stagedBytes []byte) error {
	if !lowerHex64(record.StageID) || record.StageID != filepath.Base(dir) || record.AppHash != staged.AppHash || record.ReleaseHash != staged.ReleaseHash || record.Version != staged.Version {
...
func readBoundFile(dir, name string, size int, digest string) ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("open staged %s without following links: %w", name, err)
	}
	...
	body, err := io.ReadAll(io.LimitReader(f, int64(size)+1))
	if err != nil {
		return nil, fmt.Errorf("read staged %s: %w", name, err)
	}
	if len(body) != size || !digestMatches(body, digest) {
