...
func TestReconcileRefusesStageLedgerFromAnotherDirectoryWithoutWriting(t *testing.T) {
	...
	stage["stageId"] = strings.Repeat("f", 64)
	mutatedStage := marshalLine(t, stage)
	writeTestFile(t, stageJSON, mutatedStage)
	stagedBefore, err := os.ReadFile(stagePath)
	if err != nil {
		t.Fatal(err)
	}

	if err := run([]string{"--stage", stagePath, "--final", finalPath}, ioDiscard{}); err == nil || !strings.Contains(err.Error(), "stage ledger identity") {
		t.Fatalf("cross-directory stage ledger accepted: %v", err)
	}
	stagedAfter, err := os.ReadFile(stagePath)
	if err != nil {
		t.Fatal(err)
	}
	if !bytes.Equal(stagedAfter, stagedBefore) {
		t.Fatal("refused reconciliation modified staged release")
	}
	stageAfter, err := os.ReadFile(stageJSON)
	if err != nil {
		t.Fatal(err)
	}
	if !bytes.Equal(stageAfter, mutatedStage) {
		t.Fatal("refused reconciliation modified mismatched stage ledger")
	}
}

func TestFinalizationJournalRefusesEmbeddedStageIdentityMismatch(t *testing.T) {
	root := t.TempDir()
	stageID := strings.Repeat("a", 64)
	if err := os.Mkdir(filepath.Join(root, stageID), 0o700); err != nil {
		t.Fatal(err)
	}
	beforeRelease := marshalLine(t, map[string]any{"licenseSquadsVault": ""})
	afterRelease := marshalLine(t, map[string]any{"licenseSquadsVault": "vault"})
	stageFor := func(release []byte) []byte {
		digest := sha256.Sum256(release)
		return marshalLine(t, map[string]any{
			"stageId":       strings.Repeat("b", 64),
			"releaseSha256": hex.EncodeToString(digest[:]),
			"releaseSize":   len(release),
		})
	}
	if err := stagefinalization.Prepare(root, stageID, beforeRelease, stageFor(beforeRelease), afterRelease, stageFor(afterRelease)); err == nil || !strings.Contains(err.Error(), "stage identity") {
		t.Fatalf("journal accepted cross-stage transition: %v", err)
	}
	entries, err := os.ReadDir(root)
	if err != nil {
		t.Fatal(err)
	}
	if len(entries) != 1 || entries[0].Name() != stageID {
		t.Fatalf("refused transition left a journal: %v", entries)
	}
}
