	...
	for _, flag := range []string{"--prepare-out", "--prepared-submission"} {
		t.Run(flag, func(t *testing.T) {
			...
			if _, err := parseFlags(args); err == nil {
				t.Fatal("accepted two different signed handoff modes in one request")
			}
		})
	}
...
func TestBuildEnvelopePurposeBindsStageAndPromoteBidirectionally(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			// ExpectedSignerPubkeyB58 + NonceCache are set to the otherwise-correct
			// values so the ONLY thing under test is the retargeting.
			tampered := signed
