...
func jsonPublishBody(t *testing.T, sig envelope.Signed, release, spk, metadata []byte, runtimeOverride ...[]byte) *bytes.Buffer {
	t.Helper()
	runtimeContract := runtimeContractForTest(t, spk, metadata, mustParseReleaseJSON(t, release))
...
func mustParseReleaseJSON(t *testing.T, raw []byte) ReleaseJSON {
	t.Helper()
	var rel ReleaseJSON
...
func TestHandlePublish_ReleaseEntryPDAInAllowlistDoesNotAuthorize(t *testing.T) {
	...
	// Two-phase contract: a promotion is only reachable for a candidate that was
	// durably staged first, so the control stages then promotes. The assertion is
	// unchanged — the identical publish, keyed by SIGNING KEY, must reach 200.
	w2 := stageThenPromote(t, svc2, pub, op.Public(), f.spk, release, func(sig envelope.Signed) *bytes.Buffer {
