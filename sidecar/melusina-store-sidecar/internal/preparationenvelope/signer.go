// Package preparationenvelope implements the narrow custody boundary used by a
// Bazaar Control preparer.  It is deliberately not an HTTP service, a
// transaction signer, or a release client: it signs exactly one short-lived
// private-stage envelope for a reviewed private staging dossier.
package preparationenvelope
...
const (
	RequestSchema  = "bazaar-control-preparation-envelope-request-v1"
	ResponseSchema = "bazaar-control-preparation-envelope-response-v1"
...
// Request contains facts that the preparer has already verified against its
// persisted job, prepared author state, and immutable candidate. It contains no
...
// preparer still constructs and returns the complete sidecar body; it never
...
// the service unit, rather than being supplied in a preparation job.
type Config struct {
	SocketPath            string
...
func (s *Service) Sign(request Request) (Response, error) {
	...
	now := s.now().UTC()
	target := "/control/v1/releases/" + request.DossierID + "/prepare"
...
func (r Request) validate(expectedStore string) ([]byte, error) {
	...
	if claims.Schema != "melusina-release-v1" || claims.AppHash != r.AppHash || claims.ReleaseHash != r.ReleaseHash || claims.Version != r.Version || claims.ReleaseEntryPDA != r.ReleaseEntryPDA {
		return nil, errors.New("publisher-envelope request release does not bind the prepared facts")
	}
	return release, nil
}
...
func requirePeerUID(conn *net.UnixConn, want uint32) error {
	...
	if controlErr != nil || peer == nil || peer.Uid != want {
		return errors.New("publisher-envelope signer peer is not the configured preparer user")
	}
	return nil
}
