...
func TestSignDerivesOnlyTheExactPearlPrepareRoute(t *testing.T) {
	publisher := signerIdentity(t, 1, false)
	store := signerIdentity(t, 9, true).Public()
	...
	if signed.Payload.Method != "POST" || signed.Payload.Target != "/control/v1/releases/"+request.DossierID+"/prepare" || signed.Payload.RequestHashHex != request.ArtifactSHA256 || signed.Payload.ChainEvidence.ReleaseEntryPDA != request.ReleaseEntryPDA || signed.Payload.ChainEvidence.VerifiedSlot != request.VerifiedSlot {
