...
func validateTransition(j journal) error {
	if err := validateStageID(j.StageID); err != nil {
		return err
	}
	...
	for name, stage := range map[string]map[string]json.RawMessage{
		"before": beforeStage,
		"after":  afterStage,
	} {
		var stageID string
		if err := json.Unmarshal(stage["stageId"], &stageID); err != nil || stageID != j.StageID {
			return fmt.Errorf("%s stage identity does not match journal", name)
		}
	}
...
func readRegularBounded(path string, limit int64) ([]byte, error) {
	...
	if stat.Mode&syscall.S_IFMT != syscall.S_IFREG || stat.Size < 1 || stat.Size > limit {
