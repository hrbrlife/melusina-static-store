...
func TestAcquireExistingWriterLock_IsExclusiveAndReleasedOnClose(t *testing.T) {
	...
	first, err := acquireExistingWriterLock(path)
	if err != nil {
		t.Fatalf("first acquire: %v", err)
	}
	if second, err := acquireExistingWriterLock(path); err == nil {
		_ = second.Close()
	...
	second, err := acquireExistingWriterLock(path)
	if err != nil {
...
func TestAcquireExistingWriterLock_RefusesSymlink(t *testing.T) {
	...
	if lock, err := acquireExistingWriterLock(link); err == nil {
		_ = lock.Close()
