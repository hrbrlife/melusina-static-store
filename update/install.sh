...
rerun_script_as_root() {
  ...
  if [ "$(basename $SCRIPT_NAME)" == bash ]; then
    ...
    exec sudo bash -euo pipefail -c "curl -fs -A $CURL_USER_AGENT https://bazaar.melusina-os.org/update/install.sh | $ENVVARS bash"
...
curl https://bazaar.melusina-os.org/update/install.sh > install.sh && sudo bash install.sh"
}
...
download_latest_bundle_and_extract_if_needed() {
  ...
  # install Sandstorm.
  BUILD="$(curl -A "$CURL_USER_AGENT" -fs "https://bazaar.melusina-os.org/update/$DEFAULT_UPDATE_CHANNEL")"
  ...
  do-download() {
    ...
    local URL="https://bazaar.melusina-os.org/update/sandstorm-$BUILD.tar.xz"
