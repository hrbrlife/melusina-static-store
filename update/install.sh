...
rerun_script_as_root() {
  ...
  if [ "$(basename $SCRIPT_NAME)" == bash ]; then
    ...
    exec sudo bash -euo pipefail -c "curl -fs -A $CURL_USER_AGENT __MELUSINA_STORE_PUBLIC_ORIGIN__/update/install.sh | $ENVVARS bash"
...
curl __MELUSINA_STORE_PUBLIC_ORIGIN__/update/install.sh > install.sh && sudo bash install.sh"
}
...
download_latest_bundle_and_extract_if_needed() {
  ...
  # install Sandstorm.
  BUILD="$(curl -A "$CURL_USER_AGENT" -fs "__MELUSINA_STORE_PUBLIC_ORIGIN__/update/$DEFAULT_UPDATE_CHANNEL")"
  ...
  do-download() {
    ...
    local URL="__MELUSINA_STORE_PUBLIC_ORIGIN__/update/sandstorm-$BUILD.tar.xz"
